...
use App\Models\{SubProgram, User};
...
new class extends Component {
    ...
    {
        $this->subProgram = $subProgram->load(['program.users', 'absensis.kehadirans', 'contents']);
    }
    ...
    {
        $usersQuery = $this->subProgram->program->users()
            ->where('name', 'like', '%' . $this->search . '%');

        $allUsers = $usersQuery->get();
        $totalUsers = $allUsers->count();

        // Statistik Global untuk Dashboard Index
        $avgAbsensi = 0;
        $avgProgress = 0;

        if($totalUsers > 0) {
            // Hitung rata-rata absensi seluruh mahasiswa
            $totalHadir = $this->subProgram->absensis->sum(fn($a) => $a->kehadirans->count());
            $maxPossibleHadir = $totalUsers * $this->subProgram->absensis->count();
            $avgAbsensi = $maxPossibleHadir > 0 ? round(($totalHadir / $maxPossibleHadir) * 100) : 0;
        }

        return [
            'participants' => $usersQuery->paginate(10),
            'stats' => [
                'avgAbsensi' => $avgAbsensi,
                'totalMateri' => $this->subProgram->contents->count(),
                'totalPertemuan' => $this->subProgram->absensis->count(),
                'totalPeserta' => $totalUsers
            ]
        ];
    }
}; ?>
